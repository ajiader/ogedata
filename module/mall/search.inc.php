...
if($DT_QST || !$DT_QST) {
	...
	if($dfields[$fields] == 'content') {
		...
		if($keyword) $condition .= " AND ($dfields[$fields] LIKE '%$keyword%' or jianhao LIKE '%$keyword%') "; //添加件号搜索
	...
	if($items) {
        ...
        $sql = "SELECT $fds FROM {$table} WHERE {$condition}{$order} LIMIT {$offset},{$pagesize}";
		$result = $db->query($sql, ($DT['cache_search'] && $page == 1) ? 'CACHE' : '', $DT['cache_search']);
		if($kw) {
			$replacef = explode(' ', $kw);
			$replacet = array_map('highlight', $replacef);
		}
		while($r = $db->fetch_array($result)) {
            $r['brand'] = getBrandName($r[brand_id]);
