...
require  DT_ROOT.'/module/extend/goodstype.class.php';
	...
	case 'add':
		if($submit) {
			if($do->pass($post)) {
				...
				$insertId = $do->add($post);
                for ($i=0; $i < count($attr_id_list) ; $i++) {
                    $sql = "INSERT INTO {$DT_PRE}goods_attr (goods_id,attr_id,attr_value) VALUES ('$insertId', '$attr_id_list[$i]', '$attr_value_list[$i]')";
